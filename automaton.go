...
func (r *Automaton) Copy(other *Automaton) {
	...
	r.states = slices.Grow(r.states, r.nextState+other.nextState)
	...
	r.transitions = slices.Grow(r.transitions, r.nextTransition+other.nextTransition)
...
func (r *Automaton) growStates() {
	if r.nextState+2 > len(r.states) {
		r.states = slices.Grow(r.states, r.nextState+2)
	}
}

func (r *Automaton) growTransitions() {
	if r.nextTransition+3 > len(r.transitions) {
		r.transitions = slices.Grow(r.transitions, r.nextTransition+3)
	}
}
